...
function createChart(layout, maxTime, parentSelector) {
  // Initialize D3 and a container element
  const d3n = new D3Node({
    container: layout,
    selector: parentSelector + ' .d3-vis__chart',
  });
  ...
  const y = D3Node.d3.scaleLinear()
    .domain([0, maxTime || D3Node.d3.max(data, d => d.y)]).nice()
    .range([height, 0]);
  ...
  // Reformat axes
  const yAxisEl = D3Node.d3.select(d3n.document.querySelector(parentSelector + ' .d3-vis__y-axis'));
  ...
  const xAxisEl = D3Node.d3.select(d3n.document.querySelector(parentSelector + ' .d3-vis__x-axis'));
...
// Write to file
fs.writeFile('source/partials/_time-by-date-chart.html.erb', layoutWithBothGraphs, (err) => {
